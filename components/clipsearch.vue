  ...
  <div class="h-full">
    ...
    <div class="w-full mt-32 mb-40">
      ...
      <div class="pt-8 text-center px-2">
        <span class="flex items-center justify-center text-red-500 text-xs just-nuni">
          Invalid username field !</span>
        <input v-model="cliplink"
          class="p-4 lg:w-6/12 sm:w-11/12 md:w-9/12 rounded-lg bg-gray-300 text-blue-400 just-nuni"
          type="url" name="searchclip" pattern="https://.*" placeholder="https://clips.twitch.com"/>
        <button v-on:click="getCliplink()"
...
export default {
  data() {
    return {
      card: false,
      cliplink: '',
      errStyle: ' border-2 border-red-500',
    }
  },
  methods: {
    getCliplink() {
      console.log(this.cliplink)
      this.card = !this.card
    },
  },
}
